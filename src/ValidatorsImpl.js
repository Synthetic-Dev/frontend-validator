...
import StandardValidation from "./StandardValidation.js";

export default class ValidatorsImpl {
	...
	addStandardValidation(validator, ...options) {
		return this.addItem(
			new StandardValidation(validator, this.negateNext, options)
		);
	}
	...
	isAlpha(locale, options) {
		...
		const ignore = Array.isArray(
			options === null || options === void 0 ? void 0 : options.ignore
		)
		...
		return this.addStandardValidation(
			validator.isAlpha,
			locale,
			Object.assign(Object.assign({}, options), { ignore })
		);
	}
	...
	matches(pattern, modifiers) {
		return this.addStandardValidation.apply(this, [
			...
			...(typeof pattern === "string"
				...
				: [
						pattern.source,
						[...new Set((modifiers || "") + pattern.flags)].join(
							""
						),
				  ]),
		]);
